...
If the mode is set to `kubernetes` then a normal Docker build or Dockerless JIB build is performed. The connection configuration to access the Docker daemon is described in <<access-configuration, Access Configuration>>.
...
| `JIB`
| A Dockerless https://github.com/GoogleContainerTools/jib[JIB Build] is performed when `fabric8.build.jib` property is set to true or `isJib` flag is set to true in pom.xml.Do ker daemon is not required to perform a JIB build. The generated image can be automatically pushed to the authorized registry if the credentials are correctly configured. More about https://maven.fabric8.io/#authentication[authentication] here.
|===

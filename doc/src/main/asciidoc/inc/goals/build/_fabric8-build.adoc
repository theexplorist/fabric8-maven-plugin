...
If the mode is set to `kubernetes` then a normal Docker build or Dockerless Buildah build is performed. The connection configuration to access the Docker daemon is described in <<access-configuration, Access Configuration>>.
